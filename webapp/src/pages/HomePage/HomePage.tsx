...
import { transformedOrders } from "../../utils";
      ...
      >
        <DragDropContext onDragEnd={handleDragEnd}>
          <DraggableList
            ID="0"
            listTitle="Queued"
            processItem={(order: Order) => updateOrder(order)}
            items={transformedOrders(data.Queued)}
          />
          <DraggableList
            ID="1"
            listTitle="In Progess"
            processItem={(order: Order) => updateOrder(order)}
            items={transformedOrders(data.InProgress)}
          />
          <DraggableList
            ID="2"
            listTitle="QA"
            processItem={(order: Order) => updateOrder(order)}
            items={transformedOrders(data.QA)}
          />
        </DragDropContext>
      </div>
    );
